...
def upgrade() -> None:
    ...
    op.create_primary_key("external_site_users_pkey", "external_site_users", ["id"])
    op.alter_column("external_site_users", "external_id", type_=sa.Integer(), nullable=True, index=True)
    op.alter_column(
        "external_site_users", "external_id_hash", new_column_name="id_hash", type_=sa.String(length=256), nullable=True
    )
    op.alter_column(
        "external_site_users",
        "updated_date",
...
def downgrade() -> None:
    ...
    op.drop_constraint("users_external_id_fkey", "users")
    op.drop_constraint(
        constraint_name="external_site_users_pkey",
        table_name="external_site_users",
    )
    op.drop_column("external_site_users", "id")
    op.alter_column("external_site_users", "external_id", type_=sa.Integer(), nullable=False, index=False)
    op.alter_column(
        "external_site_users",
        "id_hash",
    ...
    op.alter_column(
        "tasks", "is_archived", new_column_name="archive", type_=sa.Boolean(), server_default=None, nullable=True
    )
    ...
    op.drop_constraint("reasons_canceling_pkey", "unsubscribe_reason")
    op.create_primary_key(constraint_name="reasons_canceling_pkey", table_name="unsubscribe_reason", columns=["id"])
    op.add_column(
        "unsubscribe_reason",
    ...
    op.add_column(
        "unsubscribe_reason",
        sa.Column("reason_canceling", sa.String(length=255), nullable=True),
    )
    ...
    op.drop_constraint("users_categories_user_id_fkey", "users_categories", type_="primary")
    op.create_foreign_key(
        "users_categories_telegram_id_fkey",
        "users_categories",
        "users",
        ["telegram_id"],
        ["telegram_id"],
    ...
    op.alter_column("users", "dump_id", new_column_name="external_id")
