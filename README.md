...
3. После успешного запуска контейнеров, выполните следующую команду, которая войдет в контейнер, выполнит миграции и наполнит тестовую базу данных (включая пользователей):

    ```shell
    docker exec -it procharity_bot_backend sh -c "alembic upgrade head && python3 fill_db.py with_fake_users"
    ```

  Если тестовая база данных должна содержать только задания и не содержать пользователей, то необходимо выполнить команду:
...
3. Выполнить скрипт наполнения тестовой базы.
    ...
    Генерация тестовых данных без базы пользователей

    ```shell
    python3 fill_db.py
    ```

    Генерация тестовых данных c базой пользователей
